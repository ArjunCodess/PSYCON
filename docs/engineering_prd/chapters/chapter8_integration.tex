...
\begin{validationbox}
As of 8 September 2026, the repository implements machine-readable evidence records and ordered gate evaluation in \texttt{validation/evidence.py} and \texttt{validation/stages.py}. These controls prevent simulated results from passing physical procedures. No physical measurement records have been supplied, so all five integration stages remain open.
\end{validationbox}
...
The Week 6 live-stack runner covers normal ingestion, duplicate delivery, corrupt CRC, missing audio, buffer overrun, isolated sensor failure, temporary communication loss, watchdog recovery, safe shutdown, dashboard access, feature processing, inference, and research-export integrity. The timed stress runner labels its result as simulated and cannot satisfy physical runtime, memory, radio, thermal, electrical, or battery criteria. A fresh run of these tools was blocked because the local Docker service was unavailable; this is an environment blocker rather than physical or live-stack evidence.
