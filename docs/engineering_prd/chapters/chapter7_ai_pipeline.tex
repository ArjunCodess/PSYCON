...
The Week 3 software baseline implements a deterministic mono PCM16 extractor named \texttt{psycon\_audio\_v1}. Its ordered feature vector contains duration, RMS and peak dBFS, clipping fraction, DC offset, zero-crossing rate, spectral centroid, 85\% spectral rolloff, spectral flatness, autocorrelation fundamental frequency from 70--400~Hz, energy-active frame fraction, and pause count. This version has no downloaded-model or third-party feature-extractor dependency. Incompatible feature changes require a new extractor version and equivalence tests.

Every extracted window retains the session and device identifiers, packet sequence, device timestamp, first sample index, sample count, nominal sample rate, SHA-256 source-packet digest, and extractor version. Before inference, the pipeline returns one explicit state: usable, insufficient audio, clipped, too noisy, corrupt, or missing audio. Missing input is never replaced with zero-valued audio.
...
The completed Week 3 software adds the consent-gated \texttt{psycon\_transcription\_v1} stage. Contiguous acoustic windows marked usable are transcribed with a locally cached faster-whisper model; rejected windows never silently enter speech-to-text processing. Results preserve timestamped transcript segments, source-sample boundaries, detected language, approximate confidence, engine/model identity, and explicit complete, no-speech, skipped-quality, unavailable, not-requested, or failed states. Recordings are not sent to a third-party transcription API.

The development profile uses \texttt{small.en} on CPU with int8 computation, replacing \texttt{tiny} because transcript errors directly contaminate vocabulary, sentiment, emotion-word, and topic-transition features. The approved production profile runs \texttt{large-v3-turbo} with float16 computation on a CUDA GPU inside the PSYCON backend. \texttt{large-v3} remains a benchmark candidate rather than the production default because its additional compute and latency are justified only if representative recordings show a material accuracy or downstream-feature improvement. The model, device, and compute type are deployment configuration and must be recorded with every result.

Before release, compare \texttt{small.en}, \texttt{medium.en}, \texttt{large-v3-turbo}, and \texttt{large-v3} on representative consented recordings. Report word error rate, language-detection accuracy, end-to-end latency, peak memory, failure rate, and stability of downstream language features. Freeze a model version only after this evidence is recorded; a server deployment does not waive latency, concurrency, cost, or privacy constraints.

The versioned \texttt{psycon\_language\_v1} baseline implements the English features specified in this chapter: vocabulary diversity, sentence count and length, transparent lexicon-based sentiment, emotion-related word counts, topic-transition distance, speech duration, pause duration, and speaking rate. Unsupported languages, insufficient text, and unavailable transcription abstain explicitly. These features are intended for controlled research comparison; they are not validated emotion recognition, speaker diarization, or clinical interpretation.
