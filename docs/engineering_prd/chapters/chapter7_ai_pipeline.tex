...
The checked-in development model uses the public WESAD dataset \cite{schmidt_wesad_2018}. Its results support software development only and do not validate the PSYCON hardware or intended student-discussion population.
...
\subsection{Week 3 Speaker and Conversation Analysis}
\label{sec:week3-speaker-analysis}

The server-side \texttt{psycon\_speaker\_analysis} stage uses the locally hosted \nolinkurl{pyannote/speaker-diarization-community-1} pipeline. Regular diarization supplies overlap evidence, while exclusive diarization supplies one speaker label for transcript alignment. Faster Whisper word timestamps are assigned to the exclusive turn with at least 50\% temporal overlap; unresolved boundary words remain unknown. Pyannote model access is gated and requires an operator-provisioned Hugging Face token that must never enter source control.
...
Wearer matching is verification with abstention rather than unrestricted identity search. Each diarized cluster must contain at least three seconds of clean non-overlapping speech. Exactly one cluster is labelled \texttt{participant} only when its cosine score exceeds the configured verification threshold and its advantage over the runner-up exceeds the configured ambiguity margin. Otherwise the result is \texttt{not\_enrolled}, \texttt{not\_identified}, \texttt{ambiguous\_match}, or \texttt{insufficient\_speech}; all unmatched clusters retain recording-local anonymous labels.

Per-speaker outputs include speaking duration and share, turn count and duration, articulation rate from assigned words per active-speech minute, and session speaking rate from assigned words per recording minute. Conversation timing distinguishes within-speaker pauses of at least 200~ms, positive response gaps, signed turn-transition latency, simultaneous-speech overlap, and interruptions in which a new speaker starts at least 200~ms before the active speaker ends and continues for at least 500~ms.

Standard Praat measurements include local absolute jitter, local relative jitter, RAP, PPQ5, and DDP. Parselmouth first creates a 70--400~Hz raw-cross-correlation pitch track and then waveform-aligned pulses from the Sound and Pitch objects. The period floor is $0.8/400$, the period ceiling is $1.25/70$, and the maximum period factor is 1.3. Results retain these settings, the Parselmouth and Praat engine versions, valid-period count, region count, coverage, and rejection reasons.

Conversational measurements are explicitly labelled research estimates. Other-speaker overlap is subtracted rather than invalidating a complete turn; each remaining continuous region must last at least one second and pass energy, clipping, noise, periodicity, and pulse checks. Accepted regions are aggregated by valid period count. The deterministic acoustic extractor's existing frame-to-frame 	exttt{pitch\_jitter\_relative} feature is retained for compatibility but is not described as standard period jitter.

The preferred controlled workflow accepts an optional 3--8 second steady /a/ vowel. It removes 500~ms from both edges and selects the cleanest continuous two-second region using voiced coverage, fundamental-frequency stability, clipping, energy, and noise checks. The vowel is processed in memory and not retained. Missing or inadequate vowel input abstains without blocking conversation processing. Neither workflow applies a universal normal or pathology threshold, and neither result supports diagnosis. Hardware sampling-clock jitter requires a known-tone bench test and remains separate from vocal jitter.
...
The local webpage accepts permission-confirmed WAV, MP3, and OGG recordings, performs in-memory decoding and balanced Protocol v2 quality windows of at most two seconds, optionally runs local transcription and speaker analysis, and displays acoustic decisions, speaker-coloured turns, attributed words, language features, conversation timing, per-speaker rates, conversational jitter estimates, and provenance. It also accepts an optional sustained-vowel recording and displays its controlled jitter result separately. Separate consent is required for biometric profile comparison. Its permission checkboxes are engineering safeguards rather than a complete participant-consent system, and the localhost development server is not a production data-collection service.
