...
The local webpage accepts permission-confirmed WAV and MP3 recordings, performs in-memory decoding and balanced Protocol v2 quality windows of at most two seconds, optionally runs local transcription and speaker analysis, and displays acoustic decisions, speaker-coloured turns, attributed words, language features, conversation timing, per-speaker rates, jitter, and provenance. Separate consent is required for biometric profile comparison. Its permission checkboxes are engineering safeguards rather than a complete participant-consent system, and the localhost development server is not a production data-collection service.
