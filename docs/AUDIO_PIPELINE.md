...
Praat through Parselmouth calculates local absolute jitter, local relative jitter, RAP, PPQ5, and DDP. It first creates a raw-cross-correlation pitch track, then creates waveform-aligned pulses from the sound and pitch objects. The pitch range is 70--400 Hz, the period floor is `0.8 / 400`, the period ceiling is `1.25 / 70`, and the maximum period factor is 1.3. Results include the Parselmouth and Praat engine versions, settings, valid-period count, region count, coverage, and rejection reasons.

Conversation results are labelled `conversational_research_estimate`. Other-speaker overlap is subtracted from each turn, then each remaining continuous region must last at least one second and pass energy, clipping, noise, periodicity, and pulse checks. Accepted regions are aggregated by valid period count. The deterministic extractor's existing `pitch_jitter_relative` field is a coarse frame-to-frame F0 movement feature retained for compatibility; it is distinct from standard Praat period jitter.

The preferred controlled workflow accepts an optional 3--8 second steady `/a/` vowel. The analyzer removes the first and last 500 ms, selects the cleanest two-second region using voiced coverage, F0 stability, clipping, energy, and noise checks, and reports `controlled_sustained_vowel` results separately. The vowel remains in memory and is not stored. Neither conversational nor controlled measurements use universal normal/pathology thresholds or support diagnosis.
...
Open `http://127.0.0.1:5000`, enroll the wearer, and upload a WAV, MP3, or OGG conversation recording. Optionally upload a separate 3--8 second steady `/a/` vowel for the controlled jitter measurement. WAV follows the deterministic SciPy decoder, while MP3 and OGG are decoded in memory through PyAV. The page accepts at most 12 MB and five minutes, holds conversation, vowel, and enrollment recordings in process memory rather than writing them to disk, and provides conversation playback. It downmixes as many as eight channels, resamples supported rates to 16 kHz, converts samples to PCM16 without loudness normalization, and divides the conversation into balanced sequential windows of at most two seconds. Independent failure states keep acoustic analysis and transcription usable when model access, enrollment, or the optional vowel is missing.
